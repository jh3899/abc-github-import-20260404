...
void Abc_ShowFile( char * FileNameDot, int fKeepDot )
{
    ...
    char * pSfdpNameWin = "sfdp.exe";
    long DotFileSize = 0;
    int fUseFastLargeGraph = 0;
#else
    char FileNamePs[200];
#endif
    ...
    if ( Abc_FrameReadFlag("sfdpwin") )
        pSfdpNameWin = Abc_FrameReadFlag("sfdpwin");
    ...
    DotFileSize = ftell( pFile );
    rewind( pFile );
    fUseFastLargeGraph = DotFileSize > 200000;
#endif
    fclose( pFile );
...
#ifdef WIN32
    sprintf( FileNameSvg, "%s.svg", FileGeneric );
#else
    ...
    // check that the generated file is okay
#ifdef WIN32
    if ( (pFile = fopen( FileNameSvg, "r" )) == NULL )
    {
        fprintf( stdout, "Cannot open intermediate file \"%s\".\n", FileNameSvg );
        fprintf( stdout, "Make sure Graphviz is installed and dot.exe is available on PATH.\n" );
        return;
    }
#else
    ...
    if ( (INT_PTR)ShellExecuteA( NULL, "open", FileNameSvg, NULL, NULL, SW_SHOWNORMAL ) <= 32 )
    {
        fprintf( stdout, "Cannot open \"%s\" with the default application.\n", FileNameSvg );
        return;
    }
#else
    {
